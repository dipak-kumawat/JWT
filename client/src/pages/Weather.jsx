    ...
    <div className="flex flex-col items-center p-6">
      ...
      {weather && (
        <div className="mt-6 border rounded-lg p-4 shadow-lg w-64">
          ...
          <p className="text-sm">Temperature: {weather.current.temperature}°C</p>
          <p className="text-sm">Humidity: {weather.current.humidity}%</p>
          <p className="text-sm">Weather: {weather.current.weather_descriptions[0]}</p>
          <p className="text-sm">Wind Speed: {weather.current.wind_speed} m/s</p>
        </div>
      )}
    </div>
  );
